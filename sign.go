...
func writeHeaders(to io.Writer, headers []string, maxLen int) {
	var internalStringBuf bytes.Buffer
	var header string

	for len(headers) != 0 {
		// pop-left header
		header, headers = headers[0], headers[1:]
		bufSize := internalStringBuf.Len()
		headerSize := utf8.RuneCountInString(header)
		switch {
		case (bufSize + headerSize) <= maxLen:
			internalStringBuf.WriteString(header)
			if internalStringBuf.Len() == maxLen {
				to.Write(internalStringBuf.Bytes())
				internalStringBuf.Reset()
				internalStringBuf.WriteString(headSep)
			}
